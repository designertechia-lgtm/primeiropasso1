...
import { Sparkles, Loader2, Clapperboard, RefreshCw, Mic, Square, CheckCircle2, RotateCcw, Image as ImageIcon, Scissors, ArrowRight } from "lucide-react";
...
interface GenerateAboutVideoDialogProps {
  ...
  /** Abre o Editor completo do Estúdio Viral (com o vídeo atual, se editável).
   *  Fica no pai pra respeitar o guard de "alterações não salvas" da landing. */
  onEditInStudio: () => void;
...
export default function GenerateAboutVideoDialog({
  ...
  const [tier] = useState<Tier>(initialTier ?? "premium");
  ...
  async function gerarCriado() {
    ...
    try {
      ...
      startPolling(job_id, "Seu personagem animado está pronto e aplicado na página! 🎉");
    } catch (e: any) {
      setPhase("pronto");
      setProgress(null);
  ...
  // Polling: ao concluir, aplica o vídeo na seção Sobre e fecha o dialog.
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!gerando) onOpenChange(o); }}>
      <DialogContent className="max-w-[96vw] xl:max-w-[720px] w-full max-h-[92vh] overflow-y-auto">
        <DialogHeader>
        ...
        <div className="space-y-4">
          {/* Criar personagem (aqui) OU editar um vídeo que você já tem (abre o Editor completo) */}
          <div className="flex rounded-lg border overflow-hidden text-sm">
            <button
              type="button"
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-primary text-primary-foreground"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-background text-muted-foreground transition hover:text-foreground disabled:opacity-50"
              title="Abre o Editor completo do Estúdio Viral (corte, legendas, trilha, exportar)"
            >
              <Scissors className="h-3.5 w-3.5" /> Editar um vídeo meu
              <ArrowRight className="h-3.5 w-3.5" />
            </button>
          </div>
          <p className="-mt-2 text-xs text-muted-foreground">
            Já tem um vídeo gravado?{" "}
            <button type="button" onClick={onEditInStudio} disabled={gerando} className="font-medium text-primary underline-offset-2 hover:underline disabled:opacity-50">
              Abra no Editor
            </button>{" "}
            pra cortar, legendar e finalizar.
          </p>
