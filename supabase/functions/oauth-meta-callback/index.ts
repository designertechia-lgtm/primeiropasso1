...
// Instagram Login exige as credenciais do app do INSTAGRAM (ID/segredo próprios,
// distintos do app do Facebook). Fallback p/ META_APP_* preserva o comportamento atual.
const META_APP_ID      = Deno.env.get("INSTAGRAM_APP_ID")     ?? Deno.env.get("META_APP_ID")!;
const META_APP_SECRET  = Deno.env.get("INSTAGRAM_APP_SECRET") ?? Deno.env.get("META_APP_SECRET")!;
...
function friendlyError(raw: string): string {
  ...
  if (e.includes("professional") || e.includes("business") || e.includes("creator"))
    return "Sua conta precisa ser Profissional (Business ou Creator): Instagram → Configurações → Tipo de conta. Depois tente de novo.";
